init python:
    ...
    def increaseDef(initiator):
        ...
        if ((xSPD + xMG + xHP + xMP) / 4) > 20:
                defBonus = (xSPD + xMG + xHP + xMP)
        else:
        ...
        if not ((xSPD + xMG + xHP + xMP) / 4) > 20:
                initiator.DEF += defBonus
        initiator.MP += recMana
        #pstring02  = initiator.FTR + " is defending.  Defense increased by " + str(defBonus) +".  " + str(recMana) +" mana recovered." #WIP, fix
        pstring02  = initiator.FTR + " is defending."
